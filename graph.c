...
void findEulerianTour(struct Graph *graph) {
    ...
    if (check != 2) {
        printf("The path does not have an eulerian tour!");
        return;
    }
    ...
    while (stackIndex > 0) {
        ...
        while (node) {
            ...
            if (!visited[v * graph->V + u]) {
                stack[stackIndex++] = u;
                visited[v * graph->V + u] = 1;
                visited[u * graph->V + v] = 1;
                break;
            }
            node = node->next;
        }
...
void renderGraph(struct Graph *graph, const char *dotFileName, const char *outputFileName) {
    ...
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        snprintf(command, sizeof(command), "%s\\run_dot.bat -Tpng %s -o %s && start %s", cwd, dotFileName,
...
void createGraphFromSet(int V[], int V_size, int E[][2], int E_size, struct Graph **graph) {
    *graph = createGraph(V_size);
    for (int i = 0; i < E_size; ++i) {
        addEdge(*graph, E[i][0], E[i][1]);
    }
}